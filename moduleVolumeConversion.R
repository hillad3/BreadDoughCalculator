...
modVolumeConversionUI <- function(id_, vol_conv_){
  tagList(
    br(),
    selectInput(
    ...
    fluidRow(
      accordion(
        ...
        multiple = FALSE,
        accordion_panel(
          "Starting Volume Inputs",
          fluidRow(
            column(
              width = 2,
              numericInput(
                inputId = NS(id_,"volume_amt"),
                label = "Amount",
                value = 0,
                min = 0,
                step = 1/8
        ...
        accordion_panel(
          ...
          fluidRow(
            column(
              width = 3,
              selectInput(
                inputId = NS(id_,"target_uom"),
                label = "Unit of Measure",
                choices = c("","ounces","grams"),
                selected = "grams",
                multiple = FALSE
              )
            )
...
modVolumeConversionServer <- function(id_){
  moduleServer(
    id,
